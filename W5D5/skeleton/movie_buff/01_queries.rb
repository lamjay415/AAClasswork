...
def harrison_ford
  ...
  Movie.joins(:actors)
    .where("actors.name = 'Harrison Ford' AND castings.ord != 1").select(:id, :title)
end
...
def biggest_cast
  ...
  Movie.joins(:castings).group('movies.id').order('COUNT(movies.id) DESC').select(:id, :title).limit(3)
end
